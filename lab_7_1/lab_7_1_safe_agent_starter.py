r"""Lab 7.1 (STARTER) — Implementing Security Safeguards and Reliability Improvements: defending against the Module 6 attacks.

Run:  python lab_7_1_safe_agent_starter.py <emails_dir>
...
In Lab 6.1 we ATTACKED this very agent and found two holes:
  ...
  * Roleplay poisoning — "answer like a chicken" / "Johny-opposite" persona swaps
    hijacked the agent and corrupted every later answer.

Your job — move from DIAGNOSING those failures to IMPLEMENTING targeted, LAYERED fixes:
  * Step 1 (XML structure): wrap the retrieved e-mails and the user turn of each prompt in
    distinct XML tags (<retrieved_emails>, <user_question>). The system prompts already
    state the trust hierarchy; you build the tagged HumanMessage in plan_node and
    answer_node so the model treats only tagged database content as its source and never
    follows instructions embedded in user text. Closes the fake email injection.
  * Step 2 (input filters): implement _escape_xml (neutralize injected tags so the user
    cannot forge the structure) and _check_persona_injection (a small openai/gpt-5.4-nano
    filter that removes roleplay / ignore-instructions injections, FAIL-OPEN so a filter
    outage never breaks the agent). The regex header-strip filter (_EMAIL_HEADER_RE) and the
    filter's system prompt (_PERSONA_FILTER_SYSTEM) are already provided.
...
structure the context AND filter the input AND constrain the model with a trust
hierarchy. Robustness comes from the stack, not any one layer.
...
"chicken" roleplay) against the agent, so you can see it RESIST them once you finish the
TODOs. A fast offline self-check (_self_check) verifies your _escape_xml first.
"""
import glob
import json
import os
...
# Filter #1 (regex, cheap, deterministic) — PROVIDED: two or more standard e-mail header
# fields in sequence -> likely injected e-mail content, so strip the block.
_EMAIL_HEADER_RE = re.compile(
    r'(?:(?:From|To|Cc|Bcc|Subject|Date|Reply-To)\s*:[ \t]*[^\n]*\n){2,}',
    re.IGNORECASE,
)

# Filter #2 (a small LLM) — PROVIDED system prompt that instructs it to strip persona swaps.
...
def _get_persona_filter_llm() -> ChatOpenAI:
    """PROVIDED: build the persona-filter model lazily (on first use, not at import) so
...
def _check_persona_injection(text: str) -> str:
    ...
    TODO (Step 2): send `text` to the persona filter with _get_persona_filter_llm().invoke(
    [SystemMessage(content=_PERSONA_FILTER_SYSTEM), HumanMessage(content=text)]) and return
    the model's cleaned text (response.content, coerced to str).

    FAIL-OPEN: wrap the call in try/except and, on ANY exception, return the ORIGINAL text
    so a filter outage never breaks the agent (a deliberate availability/security tradeoff;
    a stricter deployment could fail CLOSED and reject the turn instead).

    Delete the raise NotImplementedError line once your code works.
    """
    raise NotImplementedError("Step 2: wire the persona-filter LLM call (fail-open) — see the TODO.")


def _escape_xml(text: str) -> str:
    """Escape angle brackets so user input cannot inject XML tags into the structured prompt.

    TODO (Step 2): return `text` with every '<' replaced by '&lt;' and every '>' by '&gt;'
    (str.replace is enough). This is what stops a user from forging <retrieved_emails> or
    closing a </user_question> tag to break out of their section.

    Delete the raise NotImplementedError line once your code works.
    """
    raise NotImplementedError("Step 2: escape '<' and '>' in the user text — see the TODO.")


def _sanitize_user_text(text: str) -> str:
    """PROVIDED: run all three filters over user-provided text (escape, header-strip, persona)."""
...
def _sanitize_messages(messages: list[BaseMessage]) -> list[BaseMessage]:
    """PROVIDED middleware: sanitize every HumanMessage's content before the LLM sees it.
    Wired onto the model with RunnableLambda so it runs on every invoke — defense-in-depth
    on top of the entry-point sanitization in _run()/clarify_node()/chat()."""
...
# ─── Step 1: XML-structured system prompts with an explicit trust hierarchy (PROVIDED) ──
# The prompts already TELL the model the tag structure; your job in plan_node / answer_node
# is to actually BUILD the tagged HumanMessage so these instructions have something to bind to.
...
Note that sometimes products are referred to by multiple names, so it can be useful to search for the product's alternate names and then use those names to retrieve further information. Note that in some cases the real question is in the clarifications, not in the question itself.
"""
...
class SafeResearchAgent:
    ...
    def _build_graph(self):
        ...
        def plan_node(state: _AgentState) -> dict:
            ...
            # PROVIDED: SystemMessage carries only agent instructions; prior turns are
            # proper alternating message types (not one flattened blob).
            ...
            # TODO (Step 1 — XML structure): build `current_turn` as a single string that
            # wraps each section of THIS turn in its own XML tag, then append it to
            # `messages` as a HumanMessage. Use these tags (the system prompt expects them):
            #   <queries_executed> … </queries_executed>                  from executed_str
            #   <retrieved_emails count="N"> … </retrieved_emails>        from email_summary,
            #       only when email_summary is non-empty; N = len(state["email_bodies"])
            #   <user_question> … </user_question>                        from clarif_str
            # Wrapping the user turn in its own tag is what defeats the fake-e-mail injection:
            # a pasted "e-mail block" lands inside <user_question> instead of masquerading as
            # retrieved content. After building it: messages.append(HumanMessage(content=current_turn))
            #
            # Delete the raise NotImplementedError line once you build + append current_turn.
            raise NotImplementedError("Step 1: build the XML-structured plan turn — see the TODO.")

            # PROVIDED (runs once your current_turn is appended above): ask the planner LLM.
        ...
        def answer_node(state: _AgentState) -> dict:
            ...
            # PROVIDED: SystemMessage + prior turns as alternating message types.
            ...
            # TODO (Step 1 — XML structure): build `current_turn` for the ANSWER prompt and
            # append it as a HumanMessage. Wrap the retrieved e-mails and the user question
            # in their tags:
            #   <retrieved_emails> … </retrieved_emails>   from email_context, only when non-empty
            #   <user_question> … </user_question>         from clarif_str  (always)
            # After building it: messages.append(HumanMessage(content=current_turn))
            #
            # Delete the raise NotImplementedError line once you build + append current_turn.
            raise NotImplementedError("Step 1: build the XML-structured answer turn — see the TODO.")

            # PROVIDED (runs once your current_turn is appended above): compose the answer.
            response = self._llm.invoke(messages)
...
def _self_check() -> None:
    ...
    payloads — no API key or e-mails needed. Passes once you finish _escape_xml (Step 2)."""
...
if __name__ == "__main__":
    _self_check()                 # offline — always runs (passes once Step 2 _escape_xml is done)
    require_api_key()
