r"""Lab 4.2 (STARTER) — graph-augmented retrieval over the email database.

Run:  python lab_4_2_graph_retrieval_starter.py [annotatedEmails.json]
...
Your job (see the Lab 4.2 walkthrough): implement retrievedContext() — the
graph-augmented retrieval strategy:
    hybrid search finds 5 seed e-mails
      -> for each seed, follow graph edges to add its THREAD neighbours and the
         earliest e-mails on each of its TOPICS
      -> deduplicate so no e-mail appears twice.
The graph build, the thread/topic indexes, the traversal helpers, and the context
formatter are all provided. The lesson is how to USE the graph for retrieval.

The graph is built from annotatedEmails.json, which the faculty provide. Each
record already carries sender/recipients/mentions/threadID/topics plus the raw
e-mail text, so this whole lab runs from that one file.
...
The context is organised into three kinds of e-mails:
...
# ─── Provided: data loading + hybrid stack ───────────────────────────
_STOPWORDS = {
...
# ─── Provided: build the knowledge graph ─────────────────────────────
def build_graph(emails: list[dict]) -> nx.DiGraph:
    """Directed graph with email / thread / person / topic nodes and five edge
    types: belongs_to (email->thread), sent (person->email), received_by
    (email->person), mentions (email->person), relates_to (email->topic)."""
    G = nx.DiGraph()
    for e in emails:
        fname = e["filename"]
...
class GraphRetriever(BaseRetriever):
    ...
    # — graph traversal helpers (provided) — use these in retrievedContext —
    ...
    def _closest_thread_neighbors(self, email_node: str) -> list[str]:
        ...
        expanding outward from its position and excluding the e-mail itself. Returns
        a list of filenames."""
    ...
    def _topics_for_email(self, email_node: str) -> list[str]:
        """Topic nodes this e-mail relates to."""
    ...
    def _earliest_topic_emails(self, topic_node: str) -> list[str]:
        """The TOPIC_EARLIEST earliest filenames (by seq_num) for a topic node."""
    ...
    def _assemble_context(self, seeds, thread_neighbors, topic_context) -> str:
        """Provided: format the gathered e-mails into the labelled context string."""
        sections = []
        for fname in seeds:
    ...
    def retrievedContext(self, query: str) -> str:
        ...
        TODO (Step 3): build the enriched context.
          1. seeds = [name for name, _, _ in self._hybrid.getTopK(query, HYBRID_FETCH_K)]
          2. Keep a dict  union: {filename -> source label}  to deduplicate. For each
             seed filename, set union[fname] = "seed".
          3. For each seed (skip if f"email:{fname}" is not in self._G):
               - thread_neighbors[fname] = self._closest_thread_neighbors(email_node)
                 (drop any that equal the seed), and union.setdefault(nb, "thread").
               - for each topic node in self._topics_for_email(email_node):
                     earliest = self._earliest_topic_emails(topic_node) minus the seed
                     record it under topic_context[fname][topic_name] and
                     union.setdefault(ef, "topic")  for each ef.
                 (topic_name is topic_node.removeprefix("topic:"))
          4. Optionally drop thread/topic neighbours that are themselves seeds, to
             avoid showing the same e-mail twice.
          5. return self._assemble_context(seeds, thread_neighbors, topic_context)
