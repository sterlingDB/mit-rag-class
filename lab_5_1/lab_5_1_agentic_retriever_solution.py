r"""Lab 5.1 (SOLUTION) — an agentic retriever that decides when to retrieve more.

Run:  python lab_5_1_agentic_retriever_solution.py <emails_dir>
...
The agent runs entirely inside retrievedContext(), so the rest of the chatbot
(BaseRetriever) is unchanged. Only "what to retrieve" became a decision.
...
With debug output on, you'll see each retrieval round and the agent's decision to
continue or stop. With debug output on, you’ll see each retrieval round and the agent’s decision to continue or stop. Use these observations to discuss how agentic retrieval differs from the single-shot hybrid retriever from Lab 2.2.
"""
import json
import os
import re
...
class AgenticRetriever(BaseRetriever):
    ...
    def _build_graph(self):
        ...
        def analyze_node(state: _AgentState) -> dict:
            """Ask the LLM whether the e-mails so far are enough; if not, get new queries.
            It stops automatically once max_iterations is reached, and degrades to 'done'
            if the LLM reply can't be parsed (so the agent never loops forever)."""
            if state["iterations"] >= self._max_iterations:
                return {"done": True, "pending_queries": []}

            # Cap the e-mails shown to the analyzer at 20. Every retrieval round adds more
            # e-mails, so this dictionary keeps growing; Twenty is plenty for the LLM to judge whether
            # it has enough to answer, and it keeps this decision prompt fast and within the
            # context window. The cap only bounds THIS prompt — the full set (not just 20)
            # is still returned to the chatbot at the end of retrievedContext().
            email_summary = "\n\n---\n\n".join(
                f"[{name}]\n{content}" for name, content in list(state["email_bodies"].items())[:20]
            )
            # Include the queries already executed so the analyzer targets genuinely missing
            # information and doesn't re-issue searches that were already run (this is what
            # lets the "Do not repeat queries already executed" rule in ANALYZE_SYSTEM work).
            ...
            # LLMs often wrap JSON in a markdown code fence (```json ... ```). Strip it:
            # drop the opening fence line (e.g. "```json") and everything from the closing
            # ``` onward, leaving just the JSON object for json.loads below.
            if raw.startswith("```"):
                raw = raw.split("\n", 1)[-1].rsplit("```", 1)[0]
            try:
                ...
                if self._debug:
                    print(f"[agentic] done={done} reasoning={result.get('reasoning', '')!r}")
                    if new_queries:
                        print(f"[agentic] new queries: {new_queries}")
            except (json.JSONDecodeError, ValueError):
                # If the reply isn't valid JSON, stop instead of retrying. Treating a parse
                # failure as "done" — together with the MAX_ITERATIONS cap checked above and
                # in should_continue — guarantees the retrieve->analyze loop always ends.
                done, new_queries = True, []
