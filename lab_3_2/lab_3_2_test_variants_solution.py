r"""Lab 3.2 (SOLUTION) — grow the test set with paraphrase variants.

In Lab 3.2, you probed how brittle each retriever is to rephrasings of the same
question. A systematic way to test this is to expand the test set. For each
question, ask an LLM for a few paraphrases that a real user might type, and add them
...
IMPORTANT: Automatic paraphrases can drift or produce near-duplicates. After generating the variants, review them manually and remove any that change the original meaning, introduce ambiguity, or duplicate the original or another variant. Keep only variants for which the same answer and grading notes still apply.

Run:  python lab_3_2_test_variants_solution.py [testInputs.json] [n_variants] [out.json]
...
def main():
    ...
    print(f"       python lab_3_1_evaluation_solution.py hybrid detailedEmails --inputs {out_path}")
